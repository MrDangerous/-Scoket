...
@property (weak, nonatomic) IBOutlet UITextField *recvText;
@property (weak, nonatomic) IBOutlet UITextField *connectState;
...
- (IBAction)connectClick:(UIButton *)sender {
    if ([self connection:self.hostText.text port:self.portText.text.intValue]) {
        self.connectState.text = @"connection success";
        NSLog( @"connection success");
    }
    else
    {
        self.connectState.text = @"connection error";
...
- (IBAction)sendClick:(UIButton *)sender {
    ...
    self.recvText.text = [self sendAndRecv:self.sendText.text];
}

- (IBAction)closeClick:(UIButton *)sender {
     int num = close(self.clientSocket);
    if (num == 0) {
        self.connectState.text = @"close success";
        NSLog( @"close success");
    }else{
        self.connectState.text = @"close error";
        NSLog( @"close error");
    }
}
